export default class PCPlay {
    ...
    static #potentialNextStrike(markedGrid, targetSquare) {
        ...
        // Therefore, this is done to ensure an error is no invoked
        const right =
            markedGrid[targetSquare[0]] !== undefined
                ? markedGrid[targetSquare[0]][targetSquare[1] + 1]
                : undefined;
        const left =
            markedGrid[targetSquare[0]] !== undefined
                ? markedGrid[targetSquare[0]][targetSquare[1] - 1]
                : undefined;
        const up =
            markedGrid[targetSquare[0] - 1] !== undefined
                ? markedGrid[targetSquare[0] - 1][targetSquare[1]]
                : undefined;
        const down =
            markedGrid[targetSquare[0] + 1] !== undefined
                ? markedGrid[targetSquare[0] + 1][targetSquare[1]]
        ...
        if (right === true) {
            return [targetSquare[0], targetSquare[1] + 1];
        }
        if (left === true) {
            return [targetSquare[0], targetSquare[1] - 1];
        }
        if (up === true) {
            return [targetSquare[0] - 1, [targetSquare[1]]];
        }
        if (down === true) {
            return [targetSquare[0] + 1, targetSquare[1]];
        }
    ...
    static nextMove(log, movesPerformed, sinkLog, emptyBoard) {
        const markedGrid = PCPlay.#markGrid(emptyBoard.grid, movesPerformed);
        ...
        while (logBound >= 0) {
            ...
            if (!PCPlay.inSinkRange(sinkLog, targetSquare)) {
                const potentialMove = PCPlay.#potentialNextStrike(
                    markedGrid,
                    targetSquare
                );
